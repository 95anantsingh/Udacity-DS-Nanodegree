...
def test_recs(sol_dict):
    ...
    if sol_dict['The type of recommendation system implemented here was a ...'] != d:
        print(
            "Oops!  The first key, value pair doesn't look right!  This is a collaborative filtering recommendation system based on users.")

    if sol_dict['The two methods used to estimate user similarity were: '] != e:
        print(
            "Oops! The second key, value pair doesn't look right!  Pearson's correlation coefficient and euclidean distance were used in understanding the relationships between users.")

    if sol_dict['There was an issue with using the correlation coefficient.  What was it?'] != h:
        print(
            "Oops!  The third key, value pair doesn't look right!  Because when comparing two users, there were times when the ratings were all the same.  Therefore, the standard deviation was 0, and the correlation coefficient would become a NaN value.")
...
def sim_2_sol(sol_dict):
    ...
    if pearson_dct == sol_dict:
        ...
        print(
            "Oops!  That doesn't look right... Pearson's correlation relates to a linear relationship.  The second and third cases are examples of perfect linear relationships, where we would receive values of 1 and -1.  Only having an increase or decrease that are directly related will not lead to a Pearson's correlation coefficient of 1 or -1.  You can see this by testing out your function using the examples above without using assert statements.  Try looking at the correlation of different relationships to prove the values to yourself.")
...
def sim_4_sol(sol_dict):
    ...
    if spearman_dct == sol_dict:
        ...
        print(
            "Oops!  That doesn't look right...these are actually all true statements! Unlike Pearson's correlation, Spearman's correlation can have perfect relationships (1 or -1 values) that aren't linear relationships.  You will notice that neither Spearman or Pearson correlation values suggest a relation when there are quadratic relationships.")
...
def sim_6_sol(sol_dict):
    ...
    if corr_comp_dct == sol_dict:
        ...
        print(
            "Oops!  That doesn't look right...Pearson does not match the other two measures, as it looks specifically for linear relationships.  However, Spearman and Kenall's measures are exactly the same to one another in the cases related to play_data.")
