...
def check_one(answers_one):
    ...
    if answers_one['The proportion of diabetes outcomes in the dataset'] != answers_one_1[
    ...
    if answers_one['The number of missing data points in the dataset'] != answers_one_1[
        'The number of missing data points in the dataset']:
        print(
            "Oops!  That doesn't look like the right number of missing values.  I actually couldn't find any missing values")

    if answers_one['A dataset with a symmetric distribution'] != answers_one_1[
        'A dataset with a symmetric distribution']:
        print(
            "Oops!  Of the two columns above, Glucose is actually the symmetric column.  You can see this by running the .hist() method on your dataframe.")

    if answers_one['A dataset with a right-skewed distribution'] != answers_one_1[
        'A dataset with a right-skewed distribution']:
        print(
            "Oops!  Of the two columns above, Age is actually the right-skewed column.  You can see this by running the .hist() method on your dataframe.")

    if answers_one['This variable has the strongest correlation with the outcome'] != answers_one_1[
