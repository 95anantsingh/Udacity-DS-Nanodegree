...
def no_null_cols(no_nulls):
    ...
    if no_nulls == s.no_nulls:
        ...
        print("That doesn't look like for the set of no nulls.  There should be {} columns in your list".format(
            len(s.no_nulls)))
...
def most_missing_cols(most_missing_cols):
    ...
    if most_missing_cols == s.most_missing_cols:
        ...
        print("That doesn't look like for the set of most nulls.  There should be {} columns in your list".format(
            len(s.most_missing_cols)))
...
def check_description(descrips):
    ...
    if descrips == s.descrips:
        ...
        print(
            "Though you did provide the correct data type, your result does not match what we were expecting.  Try again.\n\n  Your function should return the description for any column name passed to it.")
...
def higher_ed_test(higher_ed_perc):
    ...
    if higher_ed_perc == s.higher_ed_perc:
        print("Nice job!  That's right.  The percentage of individuals in these three groups is {}.".format(
            higher_ed_perc))
    elif val_type != float:
        print(
...
def conclusions(sol):
    ...
    if sol['There is less than a 1% difference between suggestions of the two groups for all forms of education']:
        print(
            "That's not quite right.  Almost all are less than a 1% difference.  However, there is almost a 3% difference in those that ")
    if sol['Everyone should get a higher level of formal education']:
        print(
            "That's not quite right.  The data suggests there are a number of ways to become a developer that don't require one of the categories of degree labeled as 1.")
...
def describe_check(desc_sol):
    ...
    if desc_sol == s.desc_sol:
        print("Nice job that looks right!")
    if desc_sol['A column just listing an index for each row'] != s.desc_sol[
    ...
    if desc_sol['The column with the most missing values'] != s.desc_sol['The column with the most missing values']:
        print(
            "That doesn't look like the column with the most missing values.  You can have a column appear in the dictionary more than once.")
    if desc_sol['The maximum Satisfaction on the scales for the survey'] != s.desc_sol[
        'The maximum Satisfaction on the scales for the survey']:
        print(
            "Oops! That doesn't look quite right. Are you sure that is the max value associated with the Satisfaction scales for the survey?")
    if desc_sol['The variable with the highest spread of values'] != s.desc_sol[
        'The variable with the highest spread of values']:
        print(
            "That doesn't look like the column with the largest spread.  You can have a column appear in the dictionary more than once.")
...
def scatter_check(scatter_sol):
    ...
    if scatter_sol == s.scatter_sol:
        print("Nice job that looks right!")
    if scatter_sol['The column with the strongest correlation with Salary'] != s.scatter_sol[
    ...
    if scatter_sol['The data suggests more hours worked relates to higher salary'] != s.scatter_sol[
    ...
    if scatter_sol['Data in the ______ column meant missed data in two other columns'] != s.scatter_sol[
        'Data in the ______ column meant missed data in two other columns']:
        print(
    ...
    if scatter_sol['The strongest negative relationship had what correlation?'] != s.scatter_sol[
...
def jobsat_check1(job_sol_1):
    ...
    elif job_sol_1['The proportion of missing values in the Job Satisfaction column'] != s.job_sol_1[
    ...
    elif job_sol_1['According to EmploymentStatus, which group has the highest average job satisfaction?'] != \
            s.job_sol_1['According to EmploymentStatus, which group has the highest average job satisfaction?']:
        print(
    ...
    elif job_sol_1['In general, do smaller companies appear to have employees with higher job satisfaction?'] != \
            s.job_sol_1['In general, do smaller companies appear to have employees with higher job satisfaction?']:
        print(
...
def question3_check(question3_solution):
    ...
    if question3_solution == s.question3_solution:
        ...
        print(
            "Oops!  That wasn't what we were expecting.  Your solution should be either a, b, or c for the string that best relates to what happened.")
...
def all_rm_test(all_rm):
    ...
    if all_rm.equals(s.all_rm):
        ...
        print(
            "Oops!  That doesn't look like what we were expecting.  Make sure you are working with only the numeric columns, and you have dropped any rows with missing values.")
...
def question5_check(question5_solution):
    ...
    if question5_solution == s.question5_solution:
        ...
        print(
            "Oops!  Your solution should have worked.  In which case, no output should have printed.  This solution should follow just as in the screencast.")
...
def question7_check(question7_solution):
    ...
    elif question7_solution['The number of reported salaries in the original dataset'] == s.question7_solution[
    ...
    elif question7_solution['The number of salaries predicted using our model'] == s.question7_solution[
    ...
    elif question7_solution['If an individual does not rate stackoverflow, but has a salary'] == s.question7_solution[
    ...
    elif question7_solution['If an individual does not have a a job satisfaction, but has a salary'] == \
            s.question7_solution['If an individual does not have a a job satisfaction, but has a salary']:
        print(
            "Whether an individual rates stackoverflow or has a job satisfaction we would still like to predict the salary if we can.")
    elif question7_solution['Our model predicts salaries for the two individuals described above.'] == \
            s.question7_solution['Our model predicts salaries for the two individuals described above.']:
        print(
            "Unfortunately, our current model will not predict for anyone who has missing values in any column - even if they do have a salary!")
...
def var_test(question1_solution):
    ...
    if question1_solution == s.question1_solution:
        ...
        print(
            'Oops! Two of these should be quantitative.  One we cannot know for sure. One is a category. One is a boolean.')
...
def impute_q3_check(impute_q3):
    ...
    if impute_q3 == s.impute_q3:
        ...
        print(
            'Oops! The first column looks okay, but are you sure the mean of True and False values makes sense?  Should the NaN values be True of False?  The last column is a string - what is the mean for that column?')
...
def impute_q4_check(impute_q4):
    ...
    if impute_q4 == impute_q4_1:
        ...
        print(
            "Oops! Only one of these columns actually imputed a mode.  What happened with the columns that had no mode?")
...
def check_fill_df(fill_df):
    ...
    if fill_df.equals(s.fill_df):
        ...
        print(
            "That wasn't quite as expected.  Try again, this should be the num_vars dataframe with salary removed.  Then you should fill all the numeric columns with the mean for each column.")
...
def sol_3_dict_check(sol_3_dict):
    ...
    if sol_3_dict == s.sol_3_dict:
        print('Nice job! That looks right to me!')
    elif sol_3_dict['Which column should you create a dummy variable for?'] != s.sol_3_dict[
    ...
    elif sol_3_dict['How many new dummy columns do you get when creating dummy variables?'] != s.sol_3_dict[
        'How many new dummy columns do you get when creating dummy variables?']:
        print(
            "Oops! Though you could get that number of dummy variables, that is not what you get using the default setting using one hot encoding or pandas `get_dummies` encoding. Try again.")
...
def train_test_scores_check(test_score, train_score):
    ...
    if test_score == s.test_score and train_score == s.train_score:
        ...
        print(
            "That wasn't quite as expected.  Assure you followed the steps in the document string.  The very beginning of this notebook may also be useful to assist in completing this part!")
...
def q2_piat_check(q2_piat):
    ...
    elif q2_piat['add interactions, quadratics, cubics, and other higher order terms'] != s.q2_piat[
        'add interactions, quadratics, cubics, and other higher order terms']:
        print(
    ...
    elif q2_piat['fit the model many times with different rows, then average the responses'] != s.q2_piat[
        'fit the model many times with different rows, then average the responses']:
        print(
    ...
    elif q2_piat['subset the features used for fitting the model each time'] != s.q2_piat[
        'subset the features used for fitting the model each time']:
        print(
    ...
    elif q2_piat['this model is hopeless, we should start over'] != s.q2_piat[
        'this model is hopeless, we should start over']:
...
def q4_piat_check(q4_piat):
    ...
    elif q4_piat['The optimal number of features based on the results is'] != s.q4_piat[
        'The optimal number of features based on the results is']:
        print(
    ...
    elif q4_piat['The model we should implement in practice has a train rsquared of'] != s.q4_piat[
        'The model we should implement in practice has a train rsquared of'] or q4_piat[
        'The model we should implement in practice has a test rsquared of'] != s.q4_piat[
        'The model we should implement in practice has a test rsquared of']:
        print(
            "The rsquared values don't look right.  The optimal model should be the model that performed the best on the test data.  The rsquared values should be the rsquared for the training and test sets of data using the same, best model based on the test data.")
    elif q4_piat['If we were to allow the number of features to continue to increase'] != s.q4_piat[
        'If we were to allow the number of features to continue to increase']:
        print(
            "If you were to allow the number of features to increase, you likely would see the same trend you can see in the visual.  That is the test data will continue to provide worse and worse rsquared values, while the training data would go towards 1.")
